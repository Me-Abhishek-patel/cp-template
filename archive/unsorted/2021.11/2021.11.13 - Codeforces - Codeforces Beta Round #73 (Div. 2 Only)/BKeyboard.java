...
public class BKeyboard {
    public void solve(int testNumber, InputReader in, OutputWriter out) {
        ...
        HashMap<Character, Boolean> needed = new HashMap<>();
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if (keyboard[i][j] == 'S') {
        ...
        for (Map.Entry<Character, ArrayList<IntIntPair>> e : keys.entrySet()) {
            boolean isPossible = false;
            for (IntIntPair intIntPair : e.getValue()) {
                if (getDistance(intIntPair, shiftPos, x)) {
                    isPossible = true;
                    break;
                }
            }
            if (!isPossible) needed.put(e.getKey(), false);
            else needed.put(e.getKey(), true);
        }
        int count = 0;
